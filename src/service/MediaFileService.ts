...
export default class MediaFileService extends BaseService {
  async upload(
    dto: MediaFileRequestDTO,
  ): Promise<ResponseDTO<MediaFileResponseDTO | null, string[]>> {
    let response: ResponseDTO<MediaFileResponseDTO | null, string[]> =
      new ResponseDTO(false, null, []);

    try {
      const data = await axios.post(
        URL.baseURL + "/v1/media-file",
        dto.mediaFile,
        {
          headers: {
      ...
      response.successPayload = new MediaFileResponseDTO(
        data.data.id,
        data.data.file_name,
        data.data.file_type,
        data.data.file_url,
