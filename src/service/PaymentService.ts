...
export default class PaymentService extends BaseService {
  ...
  ): Promise<ResponseDTO<CheckoutResponseDTO | null, string[]>> {
    let response: ResponseDTO<CheckoutResponseDTO | null, string[]> =
      new ResponseDTO(false, null, []);
    try {
      const data = await this._axios.get("/v1/checkout");
      response.successPayload = new CheckoutResponseDTO(
        data.data.token,
